...
def train_fn(
    ...
    for idx, (UK, japan) in enumerate(loop):
        ...
        if idx % 200 == 0:
            save_image(fake_japan * 0.5 + 0.5, f"saved_images/fake_japan_from_uk_idx{idx}.png")
            save_image(UK * 0.5 + 0.5, f"saved_images/original_uk_for_fake_japan_idx{idx}.png")

            save_image(fake_UK * 0.5 + 0.5, f"saved_images/fake_uk_from_japan_idx{idx}.png")
            save_image(japan * 0.5 + 0.5, f"saved_images/original_japan_for_fake_uk_idx{idx}.png")
...
def main():
    ...
    if hasattr(config.DEVICE, "type") and config.DEVICE.type == "cuda":
        ...
        d_scaler = torch.cuda.amp.GradScaler()
 	    ...
 	    #on MPS or CPU we use a dummy scaler that just does the operations without scaling
        class DummyScaler:
            def scale(self, loss):
                return loss
